...
async def pm_search(client, message):
 ...
 #   if user_id in ADMINS: return
    ...
    if await db.get_pm_search_status(bot_id):
        ...
        await message.reply_text("<b><i>😵‍💫ɪ ᴀᴍ ɴᴏᴛ ᴡᴏʀᴋɪɴɢ ʜᴇʀᴇ.\n\n🎭ꜱᴇᴀʀᴄʜ ᴍᴏᴠɪᴇꜱ ɪɴ ᴏᴜʀ ᴍᴏᴠɪᴇ ꜱᴇᴀʀᴄʜ ɢʀᴏᴜᴘ.</i></b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🙄 ᴍᴏᴠɪᴇ ꜱᴇᴀʀᴄʜ ɢʀᴏᴜᴘ ", url='https://t.me/codexmoviesgroup')]]))
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.append([InlineKeyboardButton(text="🎗️ ʙᴀᴄᴋ ᴛᴏ ᴍᴀɪɴ ᴘᴀɢᴇ", callback_data=f"next_{req}_{key}_{offset}")])
...
async def season_search(client: Client, query: CallbackQuery):
  ...
  #  del_msg = f"\n\n<b>🫠 ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>" if settings["auto_delete"] else ''
    ...
    btn.insert(1, [
        InlineKeyboardButton("💎 ǫᴜᴀʟɪᴛʏ 💎", callback_data=f"qualities#{key}#{offset}#{req}"),
	InlineKeyboardButton("🎗️ ꜱᴇᴀꜱᴏɴ 🎗️", callback_data=f"seasons#{key}#{offset}#{req}"),
  ...
  #  del_msg = f"\n\n<b>🫠 ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <code>{get_readable_time(DELETE_TIME)}</code> ᴛᴏ ᴀᴠᴏɪᴅ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇs</b>" if settings["auto_delete"] else ''
    ...
    if n_offset== '':
        ...
        btn.append(
            [InlineKeyboardButton("💦 ʙᴀᴄᴋ", callback_data=f"quality_search#{qul}#{key}#{offset- int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"{math.ceil(offset / int(MAX_BTN)) + 1}/{math.ceil(total / int(MAX_BTN))}", callback_data="pages",),
             InlineKeyboardButton("ɴᴇxᴛ 🌪️", callback_data=f"quality_search#{qul}#{key}#{n_offset}#{orginal_offset}#{req}"),])
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        reply_markup = InlineKeyboardMarkup(btn)
        return await query.message.edit(text=f'<b>☄️ᴛʜᴀɴᴋs ғᴏʀ ᴊᴏɪɴɪɴɢ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ 🔥😗\nɢᴇᴛ ʏᴏᴜʀ ғɪʟᴇ : {files.file_name[:20]}.. ʙʏ ᴄʟɪᴄᴋɪɴɢ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 🌪️\n\nᴍᴀɪɴᴛᴀɪɴᴇᴅ ʙʏ : @codexbotmaker</b>',reply_markup=reply_markup)
        ...
        buttons = [[
            InlineKeyboardButton('☆👀 𝙎𝙚𝙣𝙙 𝙨𝙘𝙧𝙚𝙚𝙣𝙨𝙝𝙤𝙩 👀☆', url=f'https://t.me/Codexownerr')
        ],[
            InlineKeyboardButton('💎 𝗖𝘂𝘀𝘁𝗼𝗺 𝗣𝗹𝗮𝗻 💎', callback_data='other')
        ],[
            InlineKeyboardButton('🌪️ 𝗕𝗮𝗰𝗸 🌪️', callback_data='broze'),
            InlineKeyboardButton('🧬 𝗖𝗹𝗼𝘀𝗲 🧬', callback_data='close_data')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)             
    ...
    #jisshu
       ...
       buttons = [[
            ...
            InlineKeyboardButton('🌦️ 𝗖𝗹𝗼𝘀𝗲 🌦️', callback_data='close_data')
       ]]
       reply_markup = InlineKeyboardMarkup(buttons)
    ...
    # If the user isn't an admin, return
      ...
      await client.edit_message_media(
       ...
       await client.edit_message_media(
        ...
        await query.answer("This is my details :\n\n🪃 @Codexownerr\n🎉 @codexbotmaker\n🪆 @codexmoviesgroup\n🫧 @codexdisscus\n\n\n🤗any error contact me🤗!", show_alert=True)
 
    elif query.data == "Source":
        buttons = [[
            InlineKeyboardButton('☺️ Repo loge ☺️', url='https://t.me/codexmoviesgroup')
        ],[
       ...
       buttons = [[
	      InlineKeyboardButton('👀 ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs 👀', url=f'http://t.me/{temp.U_NAME}?startgroup=start')
              ...
              InlineKeyboardButton('🎗️ ʜᴏᴍᴇ', callback_data='features'),
              InlineKeyboardButton('☺️ sᴜᴘᴘᴏʀᴛ ☺️', url=f'https://t.me/codexdisscus'),
       ]]
       reply_markup = InlineKeyboardMarkup(buttons)
       await client.edit_message_media(
          chat_id=query.message.chat.id,
        ...
        if status == "True":
            ...
            await query.answer("🌦️ ᴏɴ 🌦️")
        settings = await get_settings(int(grp_id))      
        if settings is not None:
            buttons = [[
                InlineKeyboardButton('🌪️ ᴀᴜᴛᴏ ꜰɪʟᴛᴇʀ 🌪️', callback_data=f'setgs#auto_filter#{settings["auto_filter"]}#{grp_id}'),
                InlineKeyboardButton('🌦️ ᴏɴ 🌦️' if settings["auto_filter"] else 'ᴏғғ ✗', callback_data=f'setgs#auto_filter#{settings["auto_filter"]}#{grp_id}')
            ],[
                InlineKeyboardButton('🎋 ɪᴍᴅʙ 🎋', callback_data=f'setgs#imdb#{settings["imdb"]}#{grp_id}'),
                InlineKeyboardButton('🌦️ ᴏɴ 🌦️' if settings["imdb"] else 'ᴏғғ ✗', callback_data=f'setgs#imdb#{settings["imdb"]}#{grp_id}')
            ],[
                InlineKeyboardButton('🪄 sᴘᴇʟʟ ᴄʜᴇᴄᴋ 🪄', callback_data=f'setgs#spell_check#{settings["spell_check"]}#{grp_id}'),
                InlineKeyboardButton('🌦️ ᴏɴ 🌦️' if settings["spell_check"] else 'ᴏғғ ✗', callback_data=f'setgs#spell_check#{settings["spell_check"]}#{grp_id}')
            ],[
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>sᴏʀʀʏ ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ɪs ɴᴏᴛ ᴀᴠᴀɪʟᴀʙʟᴇ ☹️</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
        ...
        if (st.status == enums.ChatMemberStatus.ADMINISTRATOR) or (st.status == enums.ChatMemberStatus.OWNER):
            ...
            await query.message.edit_reply_markup(InlineKeyboardMarkup(buttons))
            try:
                await client.send_message(chat_id=user_id, text="<b>ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛ ɪs ᴜᴘʟᴏᴀᴅᴇᴅ 🤗</b>", reply_markup=InlineKeyboardMarkup(btn))
            except UserIsBlocked:
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    ...
    if settings["link"]:
        ...
        BLOGSPOT_URL = "https://uyuyttreruiiyutyuty.blogspot.com/2025/03/redirect-portfolio.html"
        btn = [[InlineKeyboardButton(text=f"📁 {get_size(file.file_size)} ≽ {formate_file_name(file.file_name)}",url=f"{BLOGSPOT_URL}?file={file.file_id}")] 
	       for file in files
	      ]
    if offset != "":
        if total_results >= MAX_BTN:
            btn.insert(0,[
                InlineKeyboardButton("⛈️ 𝗦𝗲𝗻𝗱 𝗔𝗹𝗹 𝗙𝗶𝗹𝗲𝘀 ⛈️", callback_data=batch_link),
            ])
            btn.insert(1, [
                InlineKeyboardButton("🔮 ǫᴜᴀʟɪᴛʏ 🔮", callback_data=f"qualities#{key}#{offset}#{req}"),
                InlineKeyboardButton("🦞 ꜱᴇᴀꜱᴏɴ 🦞", callback_data=f"seasons#{key}#{offset}#{req}"),
